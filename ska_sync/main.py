...
PACKAGE_CONFIG_PATH = os.path.join(os.path.dirname(__file__), 'ska_sync.cfg')
SKA_CONFIG_PATH = os.path.join(ska_path(), 'ska_sync.cfg')  # `None` if path doesn't exist
...
def file_sync(packages, user, host):
    ...
    with open('rsync_files.tmp', 'w') as fh:
        fh.writelines(files)

    rsync_cmd = ()
    print('\n'
          'rsync -arzv --size-only --files-from=rsync_files.tmp \\\n'
          '   {user}@{host}:/proj/sot/ska/ {ska_path}/'
          .format(user=user, host=host, ska_path=ska_path()))
